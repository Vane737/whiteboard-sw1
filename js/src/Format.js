...
ArrangePanel.prototype.addGroupOps = function (div) {
  ...
  if (graph.getSelectionCount() > 1) {
    ...
    let btn = mxUtils.button(mxResources.get('ungroup'), function (evt) {
  ...
  if (graph.getSelectionCount() == 1 && graph.getModel().isVertex(cell) &&
    ...
    let btn = mxUtils.button(mxResources.get('removeFromGroup'), function (evt) {
    ...
    let btn = mxUtils.button(mxResources.get('clearWaypoints'), mxUtils.bind(this, function (evt) {
...
ArrangePanel.prototype.addAlign = function (div) {
  ...
  var left = this.editorUi.toolbar.addButton(
    'geSprite-alignleft', mxResources.get('left'),
    function () {
      graph.alignCells(mxConstants.ALIGN_LEFT);
    }, stylePanel
  );
  var center = this.editorUi.toolbar.addButton(
    'geSprite-aligncenter', mxResources.get('center'),
    function () {
      graph.alignCells(mxConstants.ALIGN_CENTER);
    }, stylePanel
  );
  var right = this.editorUi.toolbar.addButton(
    'geSprite-alignright', mxResources.get('right'),
    function () {
  ...
  var top = this.editorUi.toolbar.addButton(
    'geSprite-aligntop', mxResources.get('top'),
    function () {
      graph.alignCells(mxConstants.ALIGN_TOP);
    }, stylePanel
  );
  var middle = this.editorUi.toolbar.addButton(
    'geSprite-alignmiddle', mxResources.get('middle'),
    function () {
      graph.alignCells(mxConstants.ALIGN_MIDDLE);
    }, stylePanel
  );
  var bottom = this.editorUi.toolbar.addButton(
    'geSprite-alignbottom', mxResources.get('bottom'),
    function () {
...
TextFormatPanel.prototype.addFont = function (container) {
  ...
  var center = this.editorUi.toolbar.addButton(
    'geSprite-center', mxResources.get('center'),
    (graph.cellEditor.isContentEditing()) ?
      function () {
  ...
  if (graph.cellEditor.isContentEditing()) {
    var clear = this.editorUi.toolbar.addButton(
      'geSprite-removeformat', mxResources.get('removeFormat'),
      function () {
  ...
  if (graph.cellEditor.isContentEditing()) {
    ...
    this.styleButtons([full,
        ...
        'geSprite-subscript', mxResources.get('subscript') + ' (Ctrl+,)',
        function () {
          document.execCommand('subscript', false, null);
        }, stylePanel3
      ), sup = this.editorUi.toolbar.addButton(
        'geSprite-superscript', mxResources.get('superscript') + ' (Ctrl+.)',
        function () {
    ...
    var btns = [this.editorUi.toolbar.addButton(
    ...
    this.editorUi.toolbar.addButton(
      'geSprite-outdent', mxResources.get('decreaseIndent'),
      function () {
        document.execCommand('outdent', false, null);
      }, tmp
    ),
    this.editorUi.toolbar.addButton(
      'geSprite-indent', mxResources.get('increaseIndent'),
      function () {
        document.execCommand('indent', false, null);
      }, tmp
    ),
    this.editorUi.toolbar.addButton(
      'geSprite-code', mxResources.get('html'),
      function () {
  ...
  var inputUpdate = this.installInputHandler(
    input, mxConstants.STYLE_FONTSIZE, Menus.prototype.defaultFontSize, 1, 999, ' pt',
    function (fontsize) {
  ...
  var bgPanel = (graph.cellEditor.isContentEditing()) ? this.createColorOption(
    mxResources.get('backgroundColor'), function () {
  ...
  ) : this.createCellColorOption(mxResources.get('backgroundColor'), mxConstants.STYLE_LABEL_BACKGROUNDCOLOR, '#ffffff');
  ...
  var panel = (graph.cellEditor.isContentEditing()) ? this.createColorOption(
    mxResources.get('fontColor'), function () {
  ...
  ) : this.createCellColorOption(mxResources.get('fontColor'), mxConstants.STYLE_FONTCOLOR, '#000000', function (color) {
    if (color == null || color == mxConstants.NONE) {
  ...
  var htmlOpt = this.createCellOption(
    mxResources.get('formattedText'), 'html', '0',
  ...
  if (!graph.cellEditor.isContentEditing()) {
    ...
    var btns = [
      ...
      this.editorUi.toolbar.addButton(
        'geSprite-insertrowafter', mxResources.get('insertRowAfter'),
        function () {
          try {
            if (currentTable != null && tableRow != null) {
    ...
    var btns = [
      ...
      this.editorUi.toolbar.addButton(
        'geSprite-fit', mxResources.get('spacing'),
        function () {
          if (currentTable != null) {
...
StyleFormatPanel.prototype.addStroke = function (container) {
  ...
  function update(evt) {
    ...
    if (value != mxUtils.getValue(ss.style, mxConstants.STYLE_STROKEWIDTH, 1)) {
      ...
      ui.fireEvent(new mxEventObject(
        'styleChanged', 'keys', [mxConstants.STYLE_STROKEWIDTH],
        'values', [value], 'cells', graph.getSelectionCells()
      ));
    }
  ...
  var listener = mxUtils.bind(this, function (sender, evt, force) {
    ...
    if (es == 'orthogonalEdgeStyle' && mxUtils.getValue(ss.style, mxConstants.STYLE_CURVED, null) == '1') {
      ...
      edgeStyleDiv.className = 'geSprite ' + ((mxUtils.getValue(
        ss.style,
        mxConstants.STYLE_ELBOW, null
      ) == 'vertical') ?
...
DiagramFormatPanel.prototype.addView = function (div) {
  ...
  if (graph.isEnabled()) {
    ...
    div.appendChild(this.createOption(
      mxResources.get('guides'), function () {
    ...
    // Page View
    div.appendChild(this.createOption(
      mxResources.get('pageView'), function () {
    ...
    var bg = this.createColorOption(
      mxResources.get('background'), function () {
...
DiagramFormatPanel.prototype.addOptions = function (div) {
  ...
  if (graph.isEnabled()) {
    // Connection arrows
    div.appendChild(this.createOption(
      mxResources.get('connectionArrows'), function () {
    ...
    // Connection points
    div.appendChild(this.createOption(
      mxResources.get('connectionPoints'), function () {
...
DiagramFormatPanel.prototype.addGridOption = function (container) {
  ...
  if (mxClient.IS_SVG) {
    ...
    container.appendChild(this.createOption(
      mxResources.get('grid'), function () {
